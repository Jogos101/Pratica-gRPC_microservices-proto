...
go install google.Jogos101.org/protobuf/cmd/protoc-gen-go@latest
...
# Cria o diretório 'Jogos101', se não existir. O '-Force' evita erro se a pasta já existir.
New-Item -ItemType Directory -Path "Jogos101" -Force | Out-Null

# Comando protoc. PowerShell lida bem com as barras '/' em caminhos.
# As variáveis são referenciadas como $SERVICE_NAME
protoc --go_out=./Jogos101 `
 --go_opt=paths=source_relative `
 --go-grpc_out=./Jogos101 `
 --go-grpc_opt=paths=source_relative `
 ./${SERVICE_NAME}/*.proto

Write-Host "Arquivos Go gerados com sucesso!" -ForegroundColor Green
Get-ChildItem -Path "./Jogos101/$SERVICE_NAME" # Equivalente ao 'ls -al'
...
Set-Location -Path "./Jogos101/$SERVICE_NAME"

# Inicializa o go mod. No PowerShell, os erros não param o script por padrão,
# então o '|| true' não é necessário.
go mod init "github.com/$GITHUB_USERNAME/microservices-proto/Jogos101/$SERVICE_NAME"
go mod tidy
...
# git tag -d ch03/listing_3.2/Jogos101/${SERVICE_NAME}/${RELEASE_VERSION}
# git push --delete origin ch03/listing_3.2/Jogos101/${SERVICE_NAME}/${RELEASE_VERSION}
# git tag -fa ch03/listing_3.2/Jogos101/${SERVICE_NAME}/${RELEASE_VERSION} `
#  -m "ch03/listing_3.2/Jogos101/${SERVICE_NAME}/${RELEASE_VERSION}"
# git push origin refs/tags/ch03/listing_3.2/Jogos101/${SERVICE_NAME}/${RELEASE_VERSION}